...
const string CNPEODTask::GetTaskName()          { return "EODTask"; }

double CNPEODTask::CalculateBid(list<CNP_RobotConstraint> lstRobotConstraints) {
  ...
  */
  ...
  if (bid != 0) {
    bid = 1/sqrt(pow(targetLocation.v.x - myLocation.v.x, 2) + pow(targetLocation.v.y - myLocation.v.y, 2));
  }
