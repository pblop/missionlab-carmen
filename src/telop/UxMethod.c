/*---------------------------------------------------------------------
 * $Date: 2008/07/14 16:44:24 $             $Revision: 1.1.1.1 $
 *---------------------------------------------------------------------
 * 
 *
...
int UxPutClassCode( Widget wgt, int id)
{
    ...
    if (xcontext_mid == 0) xcontext_mid = XUniqueContext();

    if (wgt == NULL) return (UX_ERROR);

    status = XSaveContext(
	XtDisplay(UxTopLevel), 
	(Window)wgt, 
	xcontext_mid, 
	(XPointer)id);

    if (status != 0) return (UX_ERROR);

    XtAddCallback(
	wgt,
	XmNdestroyCallback, 
	UxDeleteContextCB,
	(XtPointer)xcontext_mid);
...
swidget	UxChildSite (swidget inst)
{
    ...
    swidget (*childSiteMethod) (swidget, Environment *);

    if (ChildSiteMID == -1) ChildSiteMID = UxMessageIndex("childSite");

    if (!inst) return childSite;

    // ENDO - gcc 3.4
    //(void *)childSiteMethod = UxMethodLookup (inst, ChildSiteMID, "childSite");
    childSiteMethod = (swidget(*) (swidget, Environment *))UxMethodLookup (inst, ChildSiteMID, "childSite");
    
    if (childSiteMethod)
    {
	childSite = childSiteMethod(inst, &UxEnv);

	if (childSite && childSite != inst) 
	{
	    swidget nested = UxChildSite(childSite);

	    if (nested) childSite = nested;
	}
    }
