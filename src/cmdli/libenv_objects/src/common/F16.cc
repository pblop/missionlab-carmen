 ...
 **  F16.cc                                                          **
 **                                                                  **
 **  Implement a F16-like environmental object.                      **
 **                                                                  **
...
/* $Id: F16.cc,v 1.1.1.1 2008/07/14 16:44:16 endo Exp $ */
   ...
   trail(NULL)
{
   ...
   extra = F16_extra::create(this);
}

/**********************************************************************/
// Constructor
/*
F16::F16(const Vector &location, double _roll, double _pitch, double _yaw) :
   loc(location),
   roll(_roll),
   pitch(_pitch),
   yaw(_yaw)
...
// Returns: A F16 sensor reading.
bool 
F16::create_reading(SENSOR_READING *buf, const Vector &sourceLoc, double heading)
{
/*
   buf->type = DISK_OBJECT;
   buf->SENSOR_READING_u.disk_reading.r = r;

   // Create a vector from the source to the center of the F16.
   Vector v(loc);
   v -= sourceLoc;
...
void 
F16::move_by(const Vector &distance)
{
   loc += distance;
}
...
void 
F16::trail_to(const Vector &location, uint max_length)
{
...
void 
F16::move_to(const Vector &location)
{
   loc = location;
}
...
void 
F16::rotate_by(double _roll, double _pitch, double _yaw)
{
   roll += _roll;
   pitch += _pitch;
   yaw += _yaw;
...
void 
F16::rotate_to(double _roll, double _pitch, double _yaw)
{
   roll = _roll;
   pitch = _pitch;
   yaw = _yaw;
...
void 
F16::rotateSensor_to(double yaw)
{
   sensorYaw = yaw;
}
...
double 
F16::distance(const Vector &sensorLoc)
{
   ...
   // Handle special case where the sensor is inside the F16.
   if( dist <= 0 )
      return 0;

   // return distance from edge of F16 to the source.
   return dist;
}
