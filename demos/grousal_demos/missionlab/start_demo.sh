...
OUT_FILE=/dev/null
ERROR_FILE=/dev/null
MISSION_NAME=missionlab

export IPTHOST=127.0.0.1

echo "Building robots..."
echo "This also can be done running \"cfgedit ${MISSION_NAME}.cdl\" and clicking \"Compile\""
...
rm -rf /tmp/grousal_demos/${MISSION_NAME}/tmp/
echo "Done!"
...
gnome-terminal --tab --command "hserver -a -r robot1 -f" --title=ROBOT1 \
               --tab --command "hserver -a -r robot2 -f" --title=ROBOT2 \
               --tab --command "hserver -a -r robot3 -f" --title=ROBOT3 & >$OUT_FILE 2>$ERROR_FILE
sleep 10
echo "Done!"
